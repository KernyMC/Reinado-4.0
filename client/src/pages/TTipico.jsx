...
function TTipico() {
  ...
  const verificarVotacionCompleta = async () => {
    try {
      const response = await Axios.get(
        `${API_BASE_URL}/candidatas/verificar-evento/1?userId=${currentUser.id}`
      );
      
      console.log('Respuesta del servidor:', response.data);
      
      const { completo, debeEsperar } = response.data;

      // Si debe esperar, mostrar componente de espera
      if (debeEsperar) {
        setPop(true);
        return false;
      }

      // Si todos completaron la votación
      if (completo) {
        await Axios.put(`${API_BASE_URL}/user/cambio/no/1`);
        setPop(false);
        navigate("/CRG_Gala");
        return true;
      }
  ...
  const handleClick = async () => {
    try {
      ...
      // Activar espera después de votar
      setPop(true);
      
      // Verificar inmediatamente después de votar
  ...
  // Modificar el useEffect para verificar más frecuentemente
  useEffect(() => {
    let interval;
    if (pop) {
